...
## تفعيل Canonical للحجوزات (Rollout تدريجي)

على السيرفر، بعد نجاح backfill:

```bash
cd /var/www/nasaq
DOTENV_CONFIG_PATH=/var/www/nasaq/.env DRY_RUN=true pnpm --filter @nasaq/api exec tsx ../../scripts/backfill-canonical-bookings.ts
DOTENV_CONFIG_PATH=/var/www/nasaq/.env pnpm --filter @nasaq/api exec tsx ../../scripts/backfill-canonical-bookings.ts
```

ثم فعّل الفلاجز تدريجيًا داخل `.env`:

```env
ENABLE_CANONICAL_SHADOW_WRITE=true
ENABLE_CANONICAL_READ_DETAIL=true
ENABLE_CANONICAL_READ_LIST=false
```

بعد 24 ساعة مراقبة بدون أخطاء:

```env
ENABLE_CANONICAL_READ_LIST=true
```
