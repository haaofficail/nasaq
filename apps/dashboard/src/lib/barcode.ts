// ============================================================
// BARCODE UTILITIES — أدوات الباركود الأمامية
//
// توليد SVG لعرض باركود Code128
// مسح بالكاميرا عبر BarcodeDetector API أو HTML5 input
// ============================================================
...
for (let i = 0; i < CODE128_CHARS.length; i++) {
  CODE128_B[CODE128_CHARS[i]] = i + 32;
}
...
export function generateCode128Svg(text: string, options?: { height?: number; showText?: boolean; moduleWidth?: number }): string {
  ...
  const codeValues: number[] = [startCode];

  for (let i = 0; i < text.length; i++) {
    const charCode = CODE128_B[text[i]];
    if (charCode === undefined) continue; // skip unsupported chars
    codeValues.push(charCode);
    checksum += charCode * (i + 1);
  }

  codeValues.push(checksum % 103); // check digit
  codeValues.push(106); // STOP

  // Build bar pattern
  const bars: string[] = codeValues.map(v => CODE128_PATTERNS[v] ?? "");
  const fullPattern = bars.join("");

  // Build SVG rects
  let x = 10;
  const rects: string[] = [];
  for (const bit of fullPattern) {
    if (bit === "1") {
  ...
  const textEl = showText
    ? `<text x="${totalWidth / 2}" y="${height + 14}" text-anchor="middle" font-family="monospace" font-size="12" fill="black">${text}</text>`
    : "";
...
// ── Generate a new unique barcode string (frontend helper) ────
