        ...
        {[{ id: "basic", nameAr: "الأساسي" }, { id: "advanced", nameAr: "المتقدم" }, { id: "pro", nameAr: "الاحترافي" }, { id: "enterprise", nameAr: "المؤسسي" }, ...plans.filter(p => !["basic","advanced","pro","enterprise"].includes(p.id))].map((pl: any) => (
          <button key={pl.id} onClick={() => setSelectedPlan(pl.id)}
            className={clsx("px-4 py-1.5 rounded-full text-sm font-medium border transition-all",
              selectedPlan === pl.id ? "bg-brand-500 text-white border-brand-500" : "border-[#eef2f6] text-gray-600 hover:border-brand-400 hover:text-brand-600")}>
      ...
      {selectedPlan && (
        <div className="space-y-5">
          <div className="bg-white rounded-2xl border border-[#eef2f6] overflow-hidden">
            <div className="px-5 py-3 border-b border-[#eef2f6] bg-[#f8fafc] flex items-center justify-between">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                      {items.map((f: any) => (
                        <label key={f.id} className={clsx("flex items-center gap-2.5 p-3 rounded-xl border cursor-pointer transition-all",
                          planFeats[f.id] ? "border-brand-200 bg-brand-50" : "border-[#eef2f6] hover:bg-[#f8fafc]")}>
          ...
          {quotas.length > 0 && (
            <div className="bg-white rounded-2xl border border-[#eef2f6] overflow-hidden">
              <div className="px-5 py-3 border-b border-[#eef2f6] bg-gray-50">
                  ...
                  <div key={q.id} className="bg-gray-50 rounded-xl p-3">
                    ...
                    <div className="flex items-center gap-2">
                      <input type="number" value={planQuotaVals[q.id] ?? q.defaultValue ?? 0}
                        ...
                        className="w-24 border border-[#eef2f6] rounded-lg px-2 py-1 text-sm text-center outline-none focus:border-brand-400" dir="ltr" />
  ...
  return (
    <div className="space-y-5">
      <div className="bg-white rounded-2xl border border-[#eef2f6] p-4">
        ...
        <div className="flex items-center gap-2 border border-[#eef2f6] rounded-xl px-3 py-2 mb-3">
          ...
          {filteredOrgs.map((org: any) => (
            <button key={org.id} onClick={() => { setSelectedOrg(org); setOverrideTab("entitlements"); }}
              className={clsx("flex items-center gap-2 p-2.5 rounded-xl border text-right transition-all",
                selectedOrg?.id === org.id ? "border-brand-300 bg-brand-50" : "border-[#eef2f6] hover:bg-[#f8fafc]")}>
      ...
      {selectedOrg && (
        <div className="space-y-4">
          <div className="flex items-center gap-3 p-4 bg-white rounded-2xl border border-[#eef2f6]">
            <div className="w-9 h-9 rounded-[10px] bg-brand-100 flex items-center justify-center text-brand-600 font-bold">{selectedOrg.name?.[0]}</div>
            <div>
            ...
            <button onClick={() => setSelectedOrg(null)} className="mr-auto p-1.5 hover:bg-[#f1f5f9] rounded-lg"><X className="w-4 h-4 text-gray-400" /></button>
          </div>
                  ...
                  {features.map((f: any) => {
                    ...
                    return (
                      <div key={f.id} className={clsx("flex items-center justify-between p-3 rounded-xl border transition-all",
                        hasManualOverride ? "border-amber-200 bg-amber-50" : "border-[#eef2f6]")}>
                ...
                {!(entitlements?.effectiveQuotas || []).length
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                    {(entitlements?.effectiveQuotas || []).map((q: any) => {
                      ...
                      return (
                        <div key={q.quotaId} className={clsx("rounded-xl p-3 border",
                          q.source === "override" ? "bg-amber-50 border-amber-200" : "bg-gray-50 border-[#eef2f6]")}>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <select value={quotaOverrideForm.quotaId} onChange={e => setQuotaOverrideForm(p => ({ ...p, quotaId: e.target.value }))}
                      className="w-full border border-[#eef2f6] rounded-xl px-3 py-2 text-sm outline-none">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input type="number" min="-1" value={quotaOverrideForm.value} onChange={e => setQuotaOverrideForm(p => ({ ...p, value: e.target.value }))}
                        className="w-full border border-[#eef2f6] rounded-xl px-3 py-2 text-sm outline-none" dir="ltr" />
                    </div>
                  ...
                  <div>
                    ...
                    <input value={quotaOverrideForm.reason} onChange={e => setQuotaOverrideForm(p => ({ ...p, reason: e.target.value }))}
                      className="w-full border border-[#eef2f6] rounded-xl px-3 py-2 text-sm outline-none" placeholder="سبب تعديل الحصة" />
                  </div>
                  <div className="flex gap-2 pt-1">
                    <button onClick={() => setQuotaOverrideOpen(false)} className="flex-1 py-2.5 border border-[#eef2f6] rounded-xl text-sm">إلغاء</button>
              ...
              {grants.length === 0 ? <Empty icon={Gift} text="لا توجد منح مجانية" /> : (
                <div className="bg-white rounded-2xl border border-[#eef2f6] overflow-hidden">
                  <table className="w-full text-sm">
                    <thead><tr className="border-b border-[#eef2f6] bg-[#f8fafc] text-xs text-gray-400">
                    ...
                    <tbody>
                      {grants.map((g: any) => (
                        <tr key={g.id} className="border-b border-gray-50 last:border-0 hover:bg-[#f8fafc]">
                          <td className="px-[10px] py-[6px] font-medium text-gray-900 text-xs">{g.nameAr}</td>
                          <td className="px-[10px] py-[6px]"><span className="px-2 py-0.5 bg-purple-50 text-purple-700 rounded-full text-[10px] font-medium">{g.type}</span></td>
                          <td className="px-[10px] py-[6px] text-gray-400 text-xs">{g.reason || "—"}</td>
                          <td className="px-[10px] py-[6px] text-gray-400 text-xs">{g.isPermanent ? "دائم" : g.endsAt ? new Date(g.endsAt).toLocaleDateString("ar") : "—"}</td>
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <input value={grantForm.nameAr} onChange={e => setGrantForm(p => ({ ...p, nameAr: e.target.value }))}
                      className="w-full border border-[#eef2f6] rounded-xl px-3 py-2 text-sm outline-none" placeholder="مثال: تقارير مجانية لمؤسس" />
                  </div>
                  <div>
                    <label className="text-xs text-gray-500 block mb-1">السبب *</label>
                    <input value={grantForm.reason} onChange={e => setGrantForm(p => ({ ...p, reason: e.target.value }))}
                      className="w-full border border-[#eef2f6] rounded-xl px-3 py-2 text-sm outline-none" placeholder="سبب المنحة" />
                  </div>
                  <div className="flex items-center gap-3">
                    <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    {!grantForm.isPermanent && (
                      <input type="date" value={grantForm.endsAt} onChange={e => setGrantForm(p => ({ ...p, endsAt: e.target.value }))}
                        className="flex-1 border border-[#eef2f6] rounded-xl px-3 py-2 text-sm outline-none" dir="ltr" />
                    )}
                  </div>
                  <div className="flex gap-2 pt-1">
                    <button onClick={() => setGrantOpen(false)} className="flex-1 py-2.5 border border-[#eef2f6] rounded-xl text-sm">إلغاء</button>
              ...
              <div className="bg-white rounded-2xl border border-[#eef2f6] p-4">
                {orgAddons.length === 0 ? <Empty icon={Package} text="لا توجد إضافات مفعّلة لهذه المنشأة" /> : (
                  <div className="space-y-2">
                    {orgAddons.map((a: any) => (
                      <div key={a.id} className="flex items-center justify-between p-3 bg-[#f8fafc] rounded-xl">
                        <div>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <input value={addonGrantForm.notes} onChange={e => setAddonGrantForm(p => ({ ...p, notes: e.target.value }))}
                      className="w-full border border-[#eef2f6] rounded-xl px-3 py-2 text-sm outline-none" placeholder="سبب المنح أو ملاحظات" />
                  </div>
                  <div className="flex gap-2 pt-1">
                    <button onClick={() => setAddonGrantOpen(false)} className="flex-1 py-2.5 border border-[#eef2f6] rounded-xl text-sm">إلغاء</button>
            ...
            <div className="bg-white rounded-2xl border border-[#eef2f6] p-5 space-y-4">
              ...
              <div><label className="text-xs text-gray-500 block mb-1">السبب (إلزامي) *</label>
                <input value={billingForm.reason} onChange={e => setBillingForm(p => ({ ...p, reason: e.target.value }))}
                  className="w-full border border-[#eef2f6] rounded-xl px-3 py-2 text-sm outline-none" placeholder="سبب تعديل الفوترة" />
              </div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
        {[
          ...
          <div key={i} className="bg-white rounded-2xl border border-[#eef2f6] p-3">
          ...
          <div className="bg-white rounded-2xl border border-[#eef2f6] overflow-hidden">
            {fLoading ? <Spinner /> : features.length === 0 ? <Empty icon={Layers} text="لا توجد ميزات مسجلة — أضف ميزتك الأولى" /> : (
              <table className="w-full text-sm">
                ...
                <tbody>
                  {features.map((f: any) => (
                    <tr key={f.id} className="border-b border-gray-50 last:border-0 hover:bg-[#f8fafc]/50">
                      <td className="py-3 px-5 font-mono text-xs text-gray-600">{f.id}</td>
                      <td className="py-[6px] px-[10px] font-medium text-gray-900">{f.nameAr || f.name}</td>
                      <td className="py-[6px] px-[10px] text-gray-400 text-xs">{groups.find((g: any) => g.id === f.groupId)?.nameAr || "—"}</td>
          ...
          <div className="bg-white rounded-2xl border border-[#eef2f6] overflow-hidden">
            {qLoading ? <Spinner /> : quotas.length === 0 ? <Empty icon={BarChart2} text="لا توجد حصص مسجلة — أضف حصتك الأولى" /> : (
              <table className="w-full text-sm">
                ...
                <tbody>
                  {quotas.map((q: any) => (
                    <tr key={q.id} className="border-b border-gray-50 last:border-0 hover:bg-[#f8fafc]/50">
                      <td className="py-3 px-5 font-mono text-xs text-gray-600">{q.id}</td>
                      <td className="py-[6px] px-[10px] font-medium text-gray-900">{q.nameAr || q.name}</td>
                      <td className="py-[6px] px-[10px] text-gray-400 text-xs">{q.unitAr || "—"}</td>
                      <td className="py-[6px] px-[10px] font-bold text-gray-700 tabular-nums">{q.defaultValue ?? "—"}</td>
                      <td className="py-[6px] px-[10px]">
        ...
        <div className="space-y-4">
          ...
          {aLoading ? <Spinner /> : addons.length === 0 ? <Empty icon={Package} text="لا توجد إضافات" /> : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {addons.map((a: any) => (
                <div key={a.id} className="bg-white rounded-2xl border border-[#eef2f6] p-5 hover:shadow-sm transition-shadow">
          ...
          {dLoading ? <Spinner /> : discounts.length === 0 ? <Empty icon={Tag} text="لا توجد خصومات" /> : (
            <div className="bg-white rounded-2xl border border-[#eef2f6] overflow-hidden">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {discounts.map((d: any) => (
                    <tr key={d.id} className="border-b border-gray-50 last:border-0 hover:bg-[#f8fafc]/50">
                      <td className="py-3 px-5 font-medium text-gray-900">{d.name}</td>
                      <td className="py-[6px] px-[10px] text-gray-500 text-xs">{DISC_TYPE_LABELS[d.type] || d.type}</td>
                      <td className="py-[6px] px-[10px] font-bold text-gray-900">{d.value}{d.type === "percentage" ? "%" : " ر.س"}</td>
                      <td className="py-[6px] px-[10px] text-gray-400 text-xs">{d.targetScope || "global"}</td>
                      <td className="py-[6px] px-[10px] text-gray-400 text-xs">{d.endsAt ? fmtDate(d.endsAt) : "بلا انتهاء"}</td>
                      <td className="py-[6px] px-[10px]">
      ...
      {subTab === "promos" && (
        <div className="space-y-4">
          ...
          {pLoading ? <Spinner /> : promos.length === 0 ? <Empty icon={Gift} text="لا توجد عروض ترويجية" /> : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {promos.map((p: any) => (
                <div key={p.id} className="bg-white rounded-2xl border border-[#eef2f6] p-5 hover:shadow-sm transition-shadow">
                ...
                <tbody>
                  {rules.map((r: any) => (
                    <tr key={r.id} className="border-b border-gray-50 last:border-0 hover:bg-[#f8fafc]/50">
                      ...
                      <td className="py-[6px] px-[10px] font-mono text-xs text-gray-500">{r.trigger}</td>
                      <td className="py-[6px] px-[10px] text-gray-400 text-xs">{r.scope}</td>
                      <td className="py-[6px] px-[10px]">
                        <span className={clsx("px-2 py-0.5 rounded-full text-[10px] font-semibold",
      ...
      <Modal open={showFeatureModal} onClose={() => setShowFeatureModal(false)} title="ميزة جديدة">
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input value={featureForm.id} onChange={e => setFeatureForm(p => ({ ...p, id: e.target.value }))}
                className="w-full border border-[#eef2f6] rounded-xl px-3 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-brand-200" dir="ltr" placeholder="feature_bookings" />
            </div>
            <div>
              ...
              <select value={featureForm.groupId} onChange={e => setFeatureForm(p => ({ ...p, groupId: e.target.value }))}
                className="w-full border border-[#eef2f6] rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-200">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input value={featureForm.nameEn} onChange={e => setFeatureForm(p => ({ ...p, nameEn: e.target.value }))}
                className="w-full border border-[#eef2f6] rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-200" dir="ltr" />
            </div>
          </div>
          ...
          <div className="flex justify-end gap-2 pt-2">
            <button onClick={() => setShowFeatureModal(false)} className="px-4 py-2 text-sm text-gray-600 hover:bg-[#f1f5f9] rounded-xl transition-colors">إلغاء</button>
      ...
      <Modal open={showQuotaModal} onClose={() => setShowQuotaModal(false)} title="حصة جديدة">
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input value={quotaForm.id} onChange={e => setQuotaForm(p => ({ ...p, id: e.target.value }))}
                className="w-full border border-[#eef2f6] rounded-xl px-3 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-brand-200" dir="ltr" placeholder="quota_users" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1.5">الاسم (عربي) *</label>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              ...
              <input value={quotaForm.unitAr} onChange={e => setQuotaForm(p => ({ ...p, unitAr: e.target.value }))}
                className="w-full border border-[#eef2f6] rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-200" placeholder="مثال: موظف" />
            </div>
            <div>
          ...
          <div className="flex justify-end gap-2 pt-2">
            <button onClick={() => setShowQuotaModal(false)} className="px-4 py-2 text-sm text-gray-600 hover:bg-[#f1f5f9] rounded-xl transition-colors">إلغاء</button>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input value={addonForm.key} onChange={e => setAddonForm(p => ({ ...p, key: e.target.value }))}
                className="w-full border border-[#eef2f6] rounded-xl px-3 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-brand-200" dir="ltr" placeholder="addon_extra_users" />
            </div>
            <div>
          ...
          <div>
            ...
            <input value={addonForm.nameAr} onChange={e => setAddonForm(p => ({ ...p, nameAr: e.target.value }))}
              className="w-full border border-[#eef2f6] rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-200" placeholder="مثال: موظفون إضافيون" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
          ...
          <div className="flex justify-end gap-2 pt-2">
            <button onClick={() => setShowAddonModal(false)} className="px-4 py-2 text-sm text-gray-600 hover:bg-[#f1f5f9] rounded-xl transition-colors">إلغاء</button>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input value={discForm.name} onChange={e => setDiscForm(p => ({ ...p, name: e.target.value }))}
                className="w-full border border-[#eef2f6] rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-200" placeholder="خصم الصيف" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1.5">النوع</label>
          ...
          <div>
            ...
            <input value={discForm.reason} onChange={e => setDiscForm(p => ({ ...p, reason: e.target.value }))}
              className="w-full border border-[#eef2f6] rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-200" placeholder="سبب إنشاء الخصم" />
          </div>
          <div className="flex justify-end gap-2 pt-2">
            <button onClick={() => setShowDiscModal(false)} className="px-4 py-2 text-sm text-gray-600 hover:bg-[#f1f5f9] rounded-xl transition-colors">إلغاء</button>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input value={promoForm.couponCode} onChange={e => setPromoForm(p => ({ ...p, couponCode: e.target.value.toUpperCase() }))}
              className="w-full border border-[#eef2f6] rounded-xl px-3 py-2 text-sm font-mono uppercase focus:outline-none focus:ring-2 focus:ring-brand-200" dir="ltr" placeholder="PROMO2026 (اختياري)" />
          </div>
          <div className="flex justify-end gap-2 pt-2">
            <button onClick={() => setShowPromoModal(false)} className="px-4 py-2 text-sm text-gray-600 hover:bg-[#f1f5f9] rounded-xl transition-colors">إلغاء</button>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input value={ruleForm.trigger} onChange={e => setRuleForm(p => ({ ...p, trigger: e.target.value }))}
              className="w-full border border-[#eef2f6] rounded-xl px-3 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-brand-200" dir="ltr" placeholder="e.g. on_subscription_renewed" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
          ...
          <div className="flex justify-end gap-2 pt-2">
            <button onClick={() => setShowRuleModal(false)} className="px-4 py-2 text-sm text-gray-600 hover:bg-[#f1f5f9] rounded-xl transition-colors">إلغاء</button>
