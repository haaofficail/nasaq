              ...
              {rows.map((r: any) => {
                ...
                return (
                  <tr key={r.id} className="border-b border-gray-50 hover:bg-gray-50 last:border-0">
                    ...
                    <td className="px-4 py-3 text-xs text-gray-500 hidden md:table-cell">{r.clientName ?? "—"}</td>
                    <td className="px-4 py-3 text-xs text-gray-500 hidden md:table-cell tabular-nums">{r.assetIds?.length ?? 0}</td>
