...
// ══════════════════════════════════════════════════════════════
// PLATFORM SESSION — admin's own WhatsApp session
// ══════════════════════════════════════════════════════════════

type PlatformWaStatus = "disconnected" | "connecting" | "qr_ready" | "connected";
...
function PlatformSessionSection() {
  const [state, setState] = useState<PlatformWaState | null>(null);
  ...
  const fetchStatus = useCallback(async () => {
    try {
      ...
      setState(res.data as PlatformWaState);
  ...
  const startConnect = async () => {
    ...
    try {
      ...
      if (!res.ok || !res.body) { toast.error("تعذّر بدء الاتصال"); return; }
        ...
        for (const line of lines) {
          ...
          try {
            ...
            if (msg.type === "qr") {
              ...
              toast.success("تم الاتصال بواتساب المنصة");
              setConnecting(false);
              return;
  ...
  const disconnect = async () => {
    ...
    try {
      ...
      fetchStatus();
    } catch { toast.error("فشل قطع الاتصال"); } finally { setActing(false); }
  };
  ...
  const statusConfig: Record<PlatformWaStatus, { label: string; cls: string }> = {
    ...
    <div className="space-y-5 max-w-xl">
      <div className="bg-white rounded-2xl border border-gray-100 p-5 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <span className={clsx("inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold", statusConfig[status].cls)}>
        ...
        <div className="flex flex-wrap gap-2 pt-1">
          {status !== "connected" && (
            <button onClick={startConnect} disabled={connecting || acting}
              className="flex items-center gap-2 bg-brand-500 hover:bg-brand-600 text-white text-sm font-medium rounded-xl px-4 py-2 transition-colors disabled:opacity-50">
          ...
          {status !== "disconnected" && (
            <button onClick={disconnect} disabled={acting}
          ...
          <button onClick={fetchStatus} className="flex items-center gap-1.5 text-xs text-gray-400 hover:text-gray-600 mr-auto transition-colors">
...
export default function WhatsAppGatewayTab() {
  ...
  return (
    <div className="space-y-6">
      ...
      {tab === "platform" && <PlatformSessionSection />}
