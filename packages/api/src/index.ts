...
try {
  // restaurant_tables: create if not exists first, then add any missing columns
  ...
  await pool.query(`ALTER TABLE restaurant_tables ADD COLUMN IF NOT EXISTS section    TEXT`);
  await pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS restaurant_tables_org_number_uq ON restaurant_tables(org_id, number)`);
...
// ── Bootstrap: جداول الوحدات الموسّعة ────────────────────────────────────
// جداول تُستخدم في routes لكن لم يُسجَّل لها migration رسمي بعد
try {
  ...
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_stock_movements_org  ON stock_movements(org_id)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_stock_movements_prod ON stock_movements(org_id, product_id)`);

  // ─── Fulfillments: تخصيص الأصول ────────────────────────────────────────
  await pool.query(`CREATE TABLE IF NOT EXISTS asset_allocations (
    id               UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
    org_id           UUID        NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
    fulfillment_id   UUID,
    booking_id       UUID,
  ...
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_asset_alloc_org  ON asset_allocations(org_id)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_asset_alloc_fulfillment ON asset_allocations(fulfillment_id)`);

  // ─── POS: إعدادات نقطة البيع ────────────────────────────────────────────
  await pool.query(`CREATE TABLE IF NOT EXISTS pos_settings (
    id             UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
    org_id         UUID        NOT NULL UNIQUE REFERENCES organizations(id) ON DELETE CASCADE,
  ...
  await pool.query(`CREATE TABLE IF NOT EXISTS message_logs (
    id              UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
    org_id          UUID        NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
  ...
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_message_logs_org  ON message_logs(org_id)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_message_logs_sent ON message_logs(org_id, sent_at DESC)`);

  await pool.query(`CREATE TABLE IF NOT EXISTS message_settings (
    id                             UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
    org_id                         UUID        NOT NULL UNIQUE REFERENCES organizations(id) ON DELETE CASCADE,
  ...
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_message_templates_org ON message_templates(org_id)`);
  ...
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_scheduled_msgs_org    ON scheduled_messages(org_id)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_scheduled_msgs_status ON scheduled_messages(status, scheduled_at)`);

  await pool.query(`CREATE TABLE IF NOT EXISTS whatsapp_sessions (
    id           UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
    org_id       UUID        NOT NULL UNIQUE REFERENCES organizations(id) ON DELETE CASCADE,
    phone        TEXT,
  ...
  await pool.query(`CREATE TABLE IF NOT EXISTS attendance_policies (
    id                        UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
    org_id                    UUID        NOT NULL UNIQUE REFERENCES organizations(id) ON DELETE CASCADE,
  ...
  await pool.query(`CREATE TABLE IF NOT EXISTS attendance_events (
    id           UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
    org_id       UUID        NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
    shift_id     UUID,
  ...
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_attendance_adj_org ON attendance_adjustment_requests(org_id)`);

  await pool.query(`CREATE TABLE IF NOT EXISTS attendance_schedule_days (
    id           UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
  ...
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_attendance_sched_days ON attendance_schedule_days(schedule_id)`);

  // ─── Bundles: اشتراكات الباقات ──────────────────────────────────────────
  await pool.query(`CREATE TABLE IF NOT EXISTS bundle_subscriptions (
    id          UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
    org_id      UUID        NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
  ...
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_bundle_subs_org      ON bundle_subscriptions(org_id)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_bundle_subs_customer ON bundle_subscriptions(org_id, customer_id)`);

  // ─── Flower: إعدادات صفحة الورد ─────────────────────────────────────────
  await pool.query(`CREATE TABLE IF NOT EXISTS flower_page_configs (
    id         UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
    org_id     UUID        NOT NULL UNIQUE REFERENCES organizations(id) ON DELETE CASCADE,
  ...
  // ─── Fulfillments: تنفيذ الحجوزات (أصول + مراحل) ──────────────────────────
  await pool.query(`CREATE TABLE IF NOT EXISTS fulfillments (
    id                      UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
    org_id                  UUID        NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
  ...
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_fulfillments_org     ON fulfillments(org_id)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_fulfillments_booking ON fulfillments(booking_id)`);

  // ─── Asset Maintenance Logs: سجل صيانة الأصول ──────────────────────────────
  await pool.query(`CREATE TABLE IF NOT EXISTS asset_maintenance_logs (
    id               UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
    org_id           UUID        NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
    asset_id         UUID,
  ...
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_asset_maint_logs_org   ON asset_maintenance_logs(org_id)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_asset_maint_logs_asset ON asset_maintenance_logs(asset_id)`);

  log.info("extended modules schema bootstrap complete");
} catch (err) {
  log.error({ err }, "extended modules schema bootstrap error — continuing startup");
}
