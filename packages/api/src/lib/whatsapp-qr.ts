...
export async function startQrSession(orgId: string): Promise<void> {
  ...
  try {
    ...
    // Fetch latest WA Web version with fallback for network failures
    ...
    const Browsers = baileysMod.Browsers ?? (baileysMod.default as any)?.Browsers;
    const browserConfig: [string, string, string] = Browsers?.ubuntu?.("Chrome") ?? ["Ubuntu", "Chrome", "22.04.4"];
