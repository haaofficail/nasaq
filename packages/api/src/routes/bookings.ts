...
bookingsRouter.post("/", async (c) => {
  ...
  if (ENABLE_CANONICAL_SHADOW_WRITE) {
    // Shadow path must stay non-blocking to avoid impacting the legacy create SLA.
    void shadowWriteCanonicalBookingOnCreate({ orgId, bookingId: booking.id, requestId }).catch((error) => {
      log.error({
        orgId,
